...
static int serial_fops_open(struct dfs_fd *fd)
{
    ...
    case O_RDWR:
        LOG_D("fops open: O_RDWR!");
		    ...
		    if(memcmp(fd->path,"/uart2",6) == 0 || memcmp(fd->path,"/uart3",6) == 0 || memcmp(fd->path,"/uart7",6) == 0 /*|| memcmp(fd->path,"/uart8",6) == 0*/)
				{
					flags = RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_INT_TX | RT_DEVICE_FLAG_RDWR;
				}
				else if(memcmp(fd->path,"/uart1",6) == 0 || memcmp(fd->path,"/uart4",6) == 0 || memcmp(fd->path,"/uart5",6) == 0 || memcmp(fd->path,"/uart6",6) == 0)
				{
					flags = RT_DEVICE_FLAG_DMA_RX | RT_DEVICE_FLAG_DMA_TX | RT_DEVICE_FLAG_RDWR;
				}
				else
				{
					flags |= RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_RDWR ;
...
//jxn: INT... really POLL 
    ...
    if (length == 0)
    {
		...
		if (HAL_UART_Transmit(&uart->handle, (uint8_t *)data, length, 500) == HAL_OK)
		{
				return length;
		}
		else
